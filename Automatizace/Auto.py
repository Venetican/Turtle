...
def db_conn_for_emails():
    ...
    relevant_rows_for_unique_turtles = pd.read_sql_query("""select * from nfturtle_io_wp2.nft_table a1 inner join nfturtle_io_wp2.form_responses a2 on a1.nft_id = a2.nft_id 
...
def downloading_images_from_db():
    ...
    with open("C:/Users/Administrator/Desktop/event_log.txt", "a") as log:
    ...
    for row in relevant_rows_initial.loc[:,"upload_path"]:
        time.sleep(3) 
        row = row.replace('"','')
        urllib.request.urlretrieve(str(row).replace('"',''),(f"C:/Users/Administrator/Desktop/Pics_to_check/{row[47:]}"))
        db_conn()
        dir_with_pics = "C:/Users/Administrator/Desktop/Pics_to_check/"
        ...
        mydb.close()
        with open("C:/Users/Administrator/Desktop/event_log.txt", "a") as log:
            log.write(str(f"-------New image {row[47:]} was downloaded to C:/Users/Administrator/Desktop/Pics_to_check/\n"))
    with open("C:/Users/Administrator/Desktop/event_log.txt", "a") as log:
...
def prepare():
    db_conn()
    loaded_model_b = load_model('C:/Users/Administrator/Desktop/DP_git/ModelA/DSR_results/final_retrained_b.h5')
    current_time = datetime.now().strftime("%m/%d/%Y, %H:%M:%S")
...
def unique():
    ...
    for row in relevant_rows_unique_part.loc[:,"os_path_of_all_pics"]:
        db_conn()
        shutil.copy(row, f"C:/Users/Administrator/Desktop/tmp_folder/{row[45:]}")
        if pd.DataFrame(data = difPy.dif('C:/Users/Administrator/Desktop/only_unique_turtles','C:/Users/Administrator/Desktop/tmp_folder').result).empty:
            ...
            time.sleep(3)
            cursor.execute(f"UPDATE nfturtle_io_wp2.form_responses SET nfturtle_io_wp2.form_responses.os_path_of_turtles = 'C:/Users/Administrator/Desktop/only_unique_turtles/{row[45:]}' WHERE nfturtle_io_wp2.form_responses.os_path_of_all_pics='{row}'")
            shutil.copy(row, f"C:/Users/Administrator/Desktop/only_unique_turtles/{row[45:]}")
            with open("C:/Users/Administrator/Desktop/event_log.txt", "a") as log:
                log.write(str(f"-------Image {row[45:]} in {row[:45]} was validated as unique and was placed to C:/Users/Administrator/Desktop/tmp_folder/\n"))     
        else:
            ...
            with open("C:/Users/Administrator/Desktop/event_log.txt", "a") as log:
                log.write(str(f"-------Image {row[45:]} in {row[:45]} was validated as non unique\n"))  
        mydb.commit()
        cursor.close()
        mydb.close()
        for root,dirs, files in os.walk("C:/Users/Administrator/Desktop/tmp_folder"):
            for file in files:
...
def assign_nft_id_for_unique():
    ...
    for row in relevant_rows_path_of_turtles.loc[:,"os_path_of_turtles"]:
        ...
        with open("C:/Users/Administrator/Desktop/event_log.txt", "a") as log:
                log.write(str(f"-------Image {row[45:]} got paired with nft_id={id_to_assign}\n"))
        mydb.commit()
        cursor.close()
        mydb.close()
    with open("C:/Users/Administrator/Desktop/event_log.txt", "a") as log:
...
def sending_email(html_pick,reciever):
  sender_email = "nfturtle.dao@gmail.com"
  receiver_email = reciever
  password = "jiwpdtyuemozjphp"
  #Create MIMEMultipart object
  msg = MIMEMultipart("related")
  msg["Subject"] = "Digiturtle recognition results"
...
def sending_unique_turtles():
    ...
    for index, row in relevant_rows_for_unique_turtles.iterrows():
        sending_email(html_unique.replace(replace_main_photo,str(row.values[10]).replace('"','')).replace(replace_nft_pic,row.values[3]).replace(replace_nft_odkaz, row.values[2]),row.values[9])
        time.sleep(2)
        cursor.execute(f"UPDATE nfturtle_io_wp2.form_responses SET nfturtle_io_wp2.form_responses.email_sent = 1 WHERE nfturtle_io_wp2.form_responses.os_path_of_all_pics='{row.values[18]}'")
        with open("C:/Users/Administrator/Desktop/event_log.txt", "a") as log:
...
def sending_non_unique_turtles():
    ...
    for index, row in relevant_rows_for_non_unique_turtles.iterrows():
        sending_email(html_non_unique.replace(replace_main_photo,str(row.values[6]).replace('"','')),row.values[5])
        time.sleep(2)
        cursor.execute(f"UPDATE nfturtle_io_wp2.form_responses SET nfturtle_io_wp2.form_responses.email_sent = 1 WHERE nfturtle_io_wp2.form_responses.os_path_of_all_pics='{row.values[14]}'")
        with open("C:/Users/Administrator/Desktop/event_log.txt", "a") as log:
